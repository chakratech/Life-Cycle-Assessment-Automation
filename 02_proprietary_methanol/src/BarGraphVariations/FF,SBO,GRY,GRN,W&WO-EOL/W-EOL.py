...
# --- 1. Data Setup ---
# Categories for the x-axis (Filtered and Reordered as requested)
categories = [
...
# Data for each series (Filtered and Reordered)
# Original indices: [Fossil (5), Soybean (0), Grey (2), Green (1)]
data = {
    'Upstream Processing': np.array([0, 2.3, 2.3, 2.9200]),
    'Conversion': np.array([3.08, 1.9, 1.9, 1.9]),
    'Pelleting': np.array([0.5, 0.5, 0.5, 0.5]),
    'EOL': np.array([2.44, 2.4, 2.40, 2.44]),
    'Carbon Uptake': np.array([0, 0, 0, -11.7])
}

# Totals to display on top of each bar (Filtered and Reordered)
# Original totals: [7.03 (Soy), -3.96 (Green), 7.11 (Grey), 5.34, 3.44, 6.02 (Fossil)]
totals = [6.02, 7.03, 7.11, -3.96]
...
# --- 2. Plotting ---
fig, ax = plt.subplots(figsize=(10, 8)) # Adjusted size slightly for fewer bars
...
plt.savefig('withEOL.png', dpi=300, bbox_inches='tight', transparent=True)
