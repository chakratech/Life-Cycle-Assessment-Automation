...
# Data for each series (Label "PHA Production" changed to "Conversion")
data = {
    'Upstream Processing': np.array([0, 2.3, 2.3, 2.9200]),
    'Conversion': np.array([3.08, 1.9, 1.9, 1.9]), # <-- Changed label
    'Pelleting': np.array([0.5, 0.5, 0.5, 0.5]),
    'Carbon Uptake': np.array([0, 0, 0, -11.7])
}

# Totals to display on top of each bar (Same as before)
totals = [3.58, 4.63, 4.71, -6.40]
x_indices = range(len(categories))

# Colors chosen to match the chart (Label "PHA Production" changed to "Conversion")
colors = {
    'Upstream Processing': '#5A9BD5',  # Blue
    'Conversion': '#8c564b',  # Brown # <-- Changed label
...
# --- 4. Legend ---
# Create the legend in the desired order (Label "PHA Production" changed to "Conversion")
legend_labels = [
    'Carbon Uptake', 'Upstream Processing',
    'Conversion', 'Pelleting' # <-- Changed label
]
ax.legend(
...
plt.savefig('withoutEOL', dpi=300, bbox_inches='tight', transparent=True)
