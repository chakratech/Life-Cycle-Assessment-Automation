...
# Data for each series (EOL added back, Grey Methanol removed)
data = {
    'Upstream Processing': np.array([0, 2.3, 2.9200]),
    'Conversion': np.array([3.08, 1.9, 1.9]),
    'Pelleting': np.array([0.5, 0.5, 0.5]),
    'EOL': np.array([2.44, 2.4, 2.44]), # <-- EOL data added
    'Carbon Uptake': np.array([0, 0, -11.7])
}

# Totals (From the version WITH EOL, Grey Methanol's total removed)
# Original totals: [6.02, 7.03, 7.11 (Grey), -3.96]
totals = [6.02, 7.03, -3.96]
x_indices = range(len(categories))

# Colors (EOL added back)
colors = {
    'Upstream Processing': '#5A9BD5',  # Blue
    'Conversion': '#8c564b',  # Brown
    'Pelleting': '#ED7D31',  # Orange-Pink
    'EOL': '#9999FF',  # Lilac # <-- EOL color added
...
# --- 4. Legend ---
# Create the legend in the desired order (EOL added)
legend_labels = [
    'Carbon Uptake', 'Upstream Processing',
    'Conversion', 'Pelleting', 'EOL' # <-- EOL label added
]
ax.legend(
...
plt.savefig('noGreyNoEOL', dpi=300, bbox_inches='tight', transparent=True)
